...
describe('CLAUDE.md transactions', () => {
    ...
    it.each(['local', 'global-overwrite', 'global-preserve'])('omits all operations and backups on an idempotent %s rerun', mode => {
        const { root, source } = fixture();
        ...
        if (beforeCompanion)
            expect(readFileSync(companion)).toEqual(beforeCompanion);
        else
            expect(nodeFs.existsSync(companion)).toBe(false);
    });
    ...
    it('reports a verified-backup write failure before mutation', () => {
        ...
        const fs = { ...nodeFs, writeFileSync(path, data, options) {
    ...
    it('rolls back a newly-created companion and cleans the failed operation temp', () => {
        ...
        let renames = 0;
        const fs = { ...nodeFs, renameSync(oldPath, newPath) {
                renames += 1;
                if (renames === 2)
...
//# sourceMappingURL=claude-md-transaction.test.js.map