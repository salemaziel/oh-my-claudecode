...
const checkpoint = { schema_version: 1, team_name: teamName, task_id: '1', worker_name: 'dead-worker', sequence: 4, task_version: 3, claim_token: token, resume_payload_hash: 'checkpoint-hash', resume_payload: { cursor: 4 }, updated_at: '2026-01-01T00:00:00.000Z' };
const liveTask = () => ({ id: '1', subject: 'Recover', description: 'task', status: 'in_progress', owner: 'dead-worker', version: 3, claim: { owner: 'dead-worker', token, leased_until: '2099-01-01T00:00:00.000Z' } });
...
function deps(tasks, sidecars = {}, writes = []) {
    return {
        ...
        withTaskClaimLock: async (_team, _id, _cwd, fn) => ({ ok: true, value: await fn() }),
        normalizeTask: (value) => value, isTerminalTaskStatus: () => false, taskFilePath: (_team, id) => id,
        writeAtomic: async (path, data) => { writes.push(path); tasks[path] = JSON.parse(data); },
        ...
        selectRecoveryCheckpoint: async () => ({ ok: true, checkpoint, path: '/checkpoint' }),
        readRecoveryCheckpoint: async () => ({ ok: true, checkpoint, path: '/checkpoint' }),
...
describe('recovery reservation claim protocol', () => {
    it('writes a sidecar before the pending+reserved task projection and replays each crash boundary safely', async () => {
        ...
        const precommitTasks = { '1': liveTask() };
        const precommitSidecars = { 'recovery:1': sidecars['recovery:1'] };
        const repaired = await requeueRecoveredTask(recoveryInput, deps(precommitTasks, precommitSidecars));
...
//# sourceMappingURL=recovery-reservation-claim.test.js.map