...
const mergeMocks = vi.hoisted(() => ({
    registerWorker: vi.fn(async (_worker) => undefined),
    unregisterWorker: vi.fn(async (_worker) => undefined),
    drainAndStop: vi.fn(async () => ({ unmerged: [] })),
    startMergeOrchestrator: vi.fn(),
}));
const cadenceMocks = vi.hoisted(() => ({
    installCommitCadence: vi.fn(async (_context) => ({ method: 'hook' })),
...
describe('runtime-v2 committed service reconciliation', () => {
    ...
    it('removes stale same-generation services after scale-down and retries retained teardown', async () => {
        ...
        cadenceMocks.installCommitCadence.mockImplementation(async (context) => (context?.workerName === 'worker-2' ? { method: 'fallback-poll' } : { method: 'hook' }));
...
//# sourceMappingURL=runtime-v2.service-repair.test.js.map