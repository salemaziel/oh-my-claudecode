...
import { MAX_TASK_RECOVERY_CHECKPOINT_BYTES, publishTaskRecoveryCheckpoint, readTaskRecoveryCheckpoint, selectTaskRecoveryCheckpoint, taskRecoveryClaimTokenHash, } from '../task-recovery-checkpoint.js';
...
const access = (current) => ({
    ...
    withTaskLock: async (_team, _task, _cwd, fn) => ({ ok: true, value: await fn() }),
});
function input(sequence = 1, resumePayload = { cursor: 4 }) {
...
describe('task recovery checkpoints', () => {
    it('authenticates publication against the exact live claim and stores it under the claim-scoped path', async () => {
        ...
        if (published.ok)
            expect(published.path).toBe(absPath(cwd, TeamPaths.checkpoint(teamName, taskId, taskRecoveryClaimTokenHash(claimToken), 1)));
    });
    it('enforces the 64 KiB payload boundary and immutable same-sequence replay/conflict', async () => {
        ...
        if (first.ok && replayed.ok)
            expect(replayed.checkpoint.updated_at).toBe(first.checkpoint.updated_at);
...
//# sourceMappingURL=task-recovery-checkpoint.test.js.map