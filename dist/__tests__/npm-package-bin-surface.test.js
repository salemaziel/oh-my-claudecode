...
import { PLUGIN_JSON_PATH, listSourceControlledPackageFiles, readMcpServersFromPath, readPluginMcpServers, referencesRootMcpConfig, referencesStandardHooksManifest, } from './npm-package-surface-helpers.js';
...
let packedPackageCache = null;
let packedPackageError = null;
let packedPackageInitialized = false;
let fixtureRootCache = null;
let packDirCache = null;
let packWorkspaceCache = null;
let tarballPathCache = null;
function readPackageJson() {
    return JSON.parse(readFileSync(PACKAGE_JSON_PATH, 'utf-8'));
}
function createIsolatedPackWorkspace(workspacePath) {
    mkdirSync(workspacePath, { recursive: true });
    const files = execFileSync('git', ['ls-files', '--cached', '--others', '--exclude-standard', '-z'], { cwd: PACKAGE_ROOT, encoding: 'utf-8' })
        .split('\0')
        .filter(Boolean);
    ...
    symlinkSync(join(PACKAGE_ROOT, 'node_modules'), join(workspacePath, 'node_modules'), process.platform === 'win32' ? 'junction' : 'dir');
}
function getPackedPackage() {
    ...
    try {
        ...
        const startedWithoutGeneratedBundles = [...GENERATED_BRIDGE_FILES].every((file) => !existsSync(join(packWorkspaceCache, file)));
        ...
        packedPackageCache = {
            files: new Set(files),
            packageJson: JSON.parse(readFileSync(join(extractedPackageRoot, 'package.json'), 'utf-8')),
            pluginJson: JSON.parse(readFileSync(join(extractedPackageRoot, '.claude-plugin', 'plugin.json'), 'utf-8')),
            mcpServers: readMcpServersFromPath(join(extractedPackageRoot, '.mcp.json')),
...
function expectedNpmShimNames(binName) {
...
describe('npm package bin surface regression', () => {
    ...
    it('rebuilds recovery CLI surfaces from source without committed bundles', () => {
        ...
        const apiHelp = execFileSync(process.execPath, [packedCli, 'team', 'api', '--help'], { cwd: tmpdir(), encoding: 'utf-8' });
        ...
        const resultHelp = execFileSync(process.execPath, [packedCli, 'team', 'api', 'read-recovery-result', '--help'], { cwd: tmpdir(), encoding: 'utf-8' });
    ...
    it('packs the complete source-controlled plugin and hook payload', () => {
        const packedFiles = packedPackageFixture.files;
        const missing = listSourceControlledPackageFiles().filter((file) => !packedFiles.has(file));
        expect(missing).toEqual([]);
    });
    it('keeps packed plugin and MCP manifests wired to exact standard entrypoints', () => {
        const sourcePluginJson = JSON.parse(readFileSync(PLUGIN_JSON_PATH, 'utf-8'));
        expect(packedPackageFixture.pluginJson).toEqual(sourcePluginJson);
        expect(referencesStandardHooksManifest(packedPackageFixture.pluginJson.hooks)).toBe(false);
        expect(referencesRootMcpConfig(packedPackageFixture.pluginJson.mcpServers)).toBe(true);
...
//# sourceMappingURL=npm-package-bin-surface.test.js.map